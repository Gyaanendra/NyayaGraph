...
const dummyImage = (i: number, w = 800, h = 600) => {
  ...
  return (
    ...
    `%3C/linearGradient%3E%3C/defs%3E%3Crect width='${w}' height='${h}' fill='%2312131a'/%3E` +
    `%3Crect x='10' y='10' width='${w - 20}' height='${h - 20}' rx='16' fill='url(%23g)' fill-opacity='0.25' stroke='%23${a}' stroke-width='3' stroke-dasharray='6 4'/%3E` +
    `%3Ctext x='50%25' y='46%25' text-anchor='middle' font-family='monospace' font-size='32' font-weight='800' fill='%23${a}'%3EDOSSIER %23${n}%3C/text%3E` +
    `%3Ctext x='50%25' y='58%25' text-anchor='middle' font-family='sans-serif' font-size='18' font-weight='600' fill='%23ffffff'%3EUNIDENTIFIED RECORD%3C/text%3E` +
    `%3C/svg%3E`
  )
    .replace(/ /g, "%20")
    .replace(/'/g, "%27")
...
const DEFAULT_ITEMS: GalleryItem[] = Array.from({ length: 12 }, (_, i) => ({
  ...
  title: `Forensic Subject #${i + 1}`,
  subtitle: "CCTNS Unidentified Dossier",
  link: "",
}));
...
export default function SphereGallery3D(props: SphereGallery3DProps) {
  const {
    ...
    hover = 200,
    rounded = 18,
    core = { coreSize: 24, coreColor: "#e0a43c66", lineColor: "#e0a43c33" },
    className,
    style,
    onSelectItem,
    minWidth = "100%",
  ...
  const items = images && images.length ? images : DEFAULT_ITEMS;
  const branches = clamp(Math.round(branchesProp || items.length), 1, MAX_BRANCHES);

  const hostRef = React.useRef<HTMLDivElement>(null);
  const canvasRef = React.useRef<HTMLCanvasElement>(null);

  const radius = RADIUS;
  const coreColor = core.coreColor ?? "#e0a43c66";
  const lineColor = core.lineColor ?? "#e0a43c33";

  const coreAlpha = parseColor(coreColor, [0.96, 0.72, 0.22, 0.4])[3];
  const lineAlpha = parseColor(lineColor, [0.96, 0.72, 0.22, 0.2])[3];
  ...
  React.useEffect(() => {
    ...
    if (!gl) {
      console.warn("[SphereGallery3D] WebGL unavailable");
      return;
    }
    ...
    const upload = (img: HTMLImageElement): WebGLTexture | null => {
      ...
      if (mip) {
        ...
        if (aniso) {
          const max = gl.getParameter((aniso as any).MAX_TEXTURE_MAX_ANISOTROPY_EXT);
          gl.texParameterf(
            gl.TEXTURE_2D,
    ...
    const release = (e: PointerEvent) => {
      ...
      const dist =
        Math.abs(e.clientX - pointer.downX) + Math.abs(e.clientY - pointer.downY);
      ...
      if (dist <= CLICK_SLOP_PX && dt <= CLICK_MS) {
        ...
        const idx = hitTest(frameBasis);
        if (idx >= 0) {
          const it = itemsRef.current[idx % itemsRef.current.length];
          if (onSelectItemRef.current) {
            onSelectItemRef.current(it, idx);
          } else if (it.link) {
    ...
    const frame = () => {
      ...
      if (L.coreSize > 0.001) {
        const c = parseColor(L.coreColor, [0.96, 0.72, 0.22, 1]);
        gl.disable(gl.BLEND);
      ...
      const drawStrings = (from: number, count: number) => {
        ...
        const c = parseColor(L.lineColor, [0.96, 0.72, 0.22, 1]);
        gl.useProgram(solidProg);
        gl.uniformMatrix4fv(sLoc.uMVP, false, viewProj);
        gl.bindBuffer(gl.ARRAY_BUFFER, lineBuf);
        gl.enableVertexAttribArray(sLoc.aPos);
